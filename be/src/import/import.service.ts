...
export class ImportService {
  ...
  async importStockIn(fileBuffer: Buffer) {
    ...
    for (const [index, row] of rows.entries()) {
      ...
      console.log(row); // log dữ liệu gốc từ Excel

      const product_id = Number(row.product_id);
      const warehouse_id = Number(row.warehouse_id);
      const quantity = Number(row.quantity);
      const manufacturer_id = row.manufacturer_id
        ? Number(row.manufacturer_id)
        : undefined;
      const note = row.note ?? '';

      console.log(
        `Parsed values -> product_id: ${product_id}, warehouse_id: ${warehouse_id}, quantity: ${quantity}, manufacturer_id: ${manufacturer_id}, note: ${note}`,
      );
    ...
    return { message: 'Import Excel stock-in thành công', total: successCount };
  }
